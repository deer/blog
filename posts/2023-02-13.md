...
description: "Following the Deno Mongoose guide, killing a stray MongoDB process on port 27017, and getting it running in Docker with auth."
...
Ok, so now everything works, except it's not rerunnable. If you try to run
`main.ts` again, you'll get an error like:

```
error: Uncaught MongoServerError: E11000 duplicate key error collection: test.dinosaurs index: name_1 dup key: { name: "Deno" }
```

Of course, because we aren't cleaning up. Let's do that. As Sony pointed out, we
can just connect to our mongodb instance and run commands manually.

This time I've looked at the
[docs](https://www.mongodb.com/docs/manual/reference/method/db.collection.remove/)
to find what I want. When I do `db.dinosaurs.find()`, I get:
